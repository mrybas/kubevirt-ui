...
async def create_vm_from_template(
    ...
    try:
        ...
        # VM guest DNS in custom VPCs is delivered two layers down:
        #   1. kube-ovn DHCP pushes `dns_server=<VpcDns VIP>` to the guest
        #      via the subnet's `spec.dhcpV4Options` (set in create_vpc).
        #   2. Kyverno's per-VPC `kubevirt-ui-vpc-dns-<vpc>` ClusterPolicy
        #      mutates virt-launcher pod's dnsConfig so the pod itself
        #      resolves through VpcDns (matters for KubeVirt's own
        #      ConfigMap/Secret lookups via kube-apiserver SNI).
        # Both happen automatically; no cloud-init munging required here.
